...
export function TouchDebugger() {
  const [log, setLog] = useState<string>("V16-C0");
  ...
  useEffect(() => {
    const handleTouchStart = (e: TouchEvent) => {
      console.log('[Capture Phase V16] 偵測到 touchstart 事件！');
      
      // 防呆：印出原始事件物件，看是不是 e.touches 異常
      if (!e.touches || e.touches.length === 0) {
        console.log('[Capture Phase V16] 警告：e.touches 為空 (可能是 iOS x:0 Bug)');
        setLog(prev => {
      ...
      const t = e.touches[0];
      console.log(`[Capture Phase V16] 座標: x=${t.clientX}, y=${t.clientY}`);
      ...
      console.log('[Capture Phase V16] 命中元素:', targetInfo);
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      console.log('[Capture Phase V16] 偵測到 touchend 事件！');
      const t = e.changedTouches[0];
      if (!t) {
        console.log('[Capture Phase V16] 警告：changedTouches[0] 為空');
        setLog(prev => {
          const base = prev.split('|')[0];
          return `${base}|TE0`;
        });
      ...
      console.log(`[Capture Phase V16] touchend 座標: x=${t.clientX}, y=${t.clientY}`);
      setLog(prev => {
        const base = prev.split('|')[0];
        return `${base}|TE`;
      });
    ...
    const handleClick = (e: MouseEvent) => {
      console.log('[Capture Phase V16] 偵測到 click 事件！');
      setClicks(c => c + 1);
      setLog(prev => {
        const base = prev.split('|')[0];
        return `${base}|CK`;
    ...
    const handleTouchCancel = (e: TouchEvent) => {
      console.log('[Capture Phase V16] 偵測到 touchcancel 事件！');
      setLog(prev => {
        const base = prev.split('|')[0];
        return `${base}|TC`;
      });
    ...
    console.log('[TouchDebugger V16] 已掛載，所有事件使用 Capture Phase');
    
    return () => {
      console.log('[TouchDebugger V16] 卸載');
