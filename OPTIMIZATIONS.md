...
| 9 | **禪模式小紫圓無法停音樂** | 根因:`useZenFlow` hook 已暴露完整 `play/pause/destroy` API(`src/lib/useZenFlow.ts` line 30-573,Howler.js 內存實例),但全檔 0 個 caller 對接 `pause()`。`FlowTimerModal.tsx:89-91` 留空 useEffect + 註解「音樂控制已從計時器拆分」,但 split 後沒人接回去。修法:FlowTimer 小紫圓 iframe wrapper 內加絕對定位 ⏸/▶ overlay 按鈕(20×20、bottom-right、桌機 hover-reveal / 手機 always-visible),`zenState.isPlaying` 雙向同步,點擊呼叫 `controller[isPlaying ? 'pause' : 'play']()`。不動 iframe、不動 API、不動 ZenDashboard 結構。build clean / 25/25 static pages | (本 commit) | 2026-07-29 |
